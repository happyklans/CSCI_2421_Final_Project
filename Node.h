...
class GeneralData //changed from struct ot a class to overload the << operator
{
public:
    int number; //Update this to your data field  
	string name;

	friend ostream & operator << (ostream & out, GeneralData item) { out << item.name << endl; return out; }
};
